...
function bucketOpenAmount(
  ...
  const ageDays = Math.floor(
    (snapshotDate.getTime() - basisDate.getTime()) / (24 * 60 * 60 * 1000),
  );
...
async function saveCash(
  ...
  const data = rows
    .map((row, index) => {
      const accountName =
        asString(row.accountName) || `Cash Account ${index + 1}`;
      return {
        companyId,
        snapshotDate,
        frequency,
...
async function saveARAging(
  ...
  await prisma.aRAgingSnapshot.upsert({
    where: {
      companyId_snapshotDate_frequency: { companyId, snapshotDate, frequency },
    },
    update: total,
...
async function saveAROpenInvoices(
  ...
  const data = rows
    .map((row, index) => {
      ...
      const buckets = bucketOpenAmount(
        amountDueHome,
        snapshotDate,
        dueDate,
        invoiceDate,
      ...
      return {
        ...
        amountCurrency:
          toNumber(row.amountCurrency) ||
          toNumber(row.amountHome) ||
          amountDueHome,
        amountHome:
          toNumber(row.amountHome) ||
          toNumber(row.amountCurrency) ||
...
async function saveARPayments(
  ...
  const data = rows
    .map((row) => {
      ...
      return paymentDate
        ? {
            ...
            currencyCode: asString(row.currencyCode) || "USD",
            paidAmountCurrency:
              toNumber(row.paidAmountCurrency) || toNumber(row.paidAmountHome),
            paidAmountHome:
              toNumber(row.paidAmountHome) || toNumber(row.paidAmountCurrency),
            sourcePlatform: "QUICKBOOKS_DESKTOP",
    ...
    .filter((row): row is NonNullable<typeof row> =>
      Boolean(row && row.paidAmountHome !== 0),
    );
    ...
    .map((row, index) => {
      ...
      return paymentDate
        ? {
            ...
            currencyCode: asString(row.currencyCode) || "USD",
            paidAmountCurrency:
              toNumber(row.paidAmountCurrency) || paidAmountHome,
            paidAmountHome,
            sourcePlatform: "QUICKBOOKS_DESKTOP",
            sourceItemId:
              asString(row.sourceItemId) ||
              `qbd|${sourceTransaction || index}|${paymentDate.toISOString()}|${vendorName}|${billNo || ""}|${paidAmountHome}`,
...
function qbdRecordKey(
  record: Record<string, unknown>,
  fallbackPrefix: string,
  index: number,
): string {
  return (
    qbdFirstString(record, "TxnID", "RefNumber", "EditSequence") ||
    `${fallbackPrefix}:${index}`
  );
}

function qbdRecordNumber(
  record: Record<string, unknown>,
  fallbackPrefix: string,
  index: number,
): string {
  return (
    qbdFirstString(record, "RefNumber", "TxnID") ||
    `${fallbackPrefix}-${index + 1}`
  );
}

function qbdRecordAmount(
  record: Record<string, unknown>,
  ...fields: string[]
): number {
  for (const field of fields) {
...
function addAppliedAmount(
  index: Map<
    string,
    Array<{ paymentDate: Date; amount: number; sourceId: string }>
  >,
...
function buildAppliedPaymentIndex(
  records: Array<Record<string, unknown>>,
  appliedType: "invoice" | "bill",
) {
  const index = new Map<
    string,
    Array<{ paymentDate: Date; amount: number; sourceId: string }>
  ...
  records.forEach((payment, paymentIndex) => {
    ...
    const paymentSourceId =
      asString(payment.TxnID) ||
      asString(payment.RefNumber) ||
      `payment:${paymentIndex}`;
    const appliedRows = Array.isArray(payment.AppliedToTxnRet)
...
export async function saveQuickBooksDesktopDetailOpenSnapshots(
  ...
  const invoices = dedupeQbdRecords(
    Array.isArray(payload.__qbdInvoices) ? payload.__qbdInvoices : [],
    "invoice",
  );
  const bills = dedupeQbdRecords(
    Array.isArray(payload.__qbdBills) ? payload.__qbdBills : [],
    "bill",
  );
  const arPaymentIndex = buildAppliedPaymentIndex(
    Array.isArray(payload.__qbdReceivePayments)
      ? payload.__qbdReceivePayments
      : [],
    "invoice",
  );
  const apPaymentIndex = buildAppliedPaymentIndex(
    Array.isArray(payload.__qbdBillPayments) ? payload.__qbdBillPayments : [],
    "bill",
  );
  ...
  for (const dateKey of businessDayKeys(startDate, endDate)) {
    ...
    invoices.forEach((invoice, index) => {
      ...
      if (
        String(invoice.IsPaid || "").toLowerCase() === "true" &&
        !hasAppliedPayments(arPaymentIndex, matchKeys)
      )
        return;
      ...
      const reportedTotal = qbdRecordAmount(
        invoice,
        "TotalAmount",
        "Subtotal",
        "Amount",
      ...
      const totalAmount = Math.max(
        currentBalance + appliedTotal,
        reportedTotal,
      );
      if (totalAmount <= 0) return;
      const paidAmount = paidThroughDate(
        arPaymentIndex,
        matchKeys,
        snapshotDate,
      );
    ...
    bills.forEach((bill, index) => {
      ...
      if (
        String(bill.IsPaid || "").toLowerCase() === "true" &&
        !hasAppliedPayments(apPaymentIndex, matchKeys)
      )
        return;
    ...
    recordsCreated += await saveAROpenInvoices(
      companyId,
      snapshotDate,
      frequency,
      arRows,
    );
    recordsCreated += await saveAPOpenBills(
      companyId,
      snapshotDate,
      frequency,
      apRows,
...
export async function syncQuickBooksDesktopOperationalPayload(
  ...
  try {
    recordsCreated += await saveCash(
      companyId,
      snapshotDate,
      frequency,
      Array.isArray(payload.cash) ? payload.cash : [],
    );
  } catch (error) {
    errors.push(
      `cash: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    recordsCreated += await saveARAging(
      companyId,
      snapshotDate,
      frequency,
      toArray(payload.arAging),
    );
  } catch (error) {
    errors.push(
      `arAging: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    recordsCreated += await saveAPAging(
      companyId,
      snapshotDate,
      frequency,
      toArray(payload.apAging),
    );
  } catch (error) {
    errors.push(
      `apAging: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    ...
    errors.push(
      `customerSales: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    ...
    errors.push(
      `productSales: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    ...
    errors.push(
      `inventory: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    if (
      payload.__qbdSourceDateRange &&
      (payload.__qbdInvoices?.length || payload.__qbdBills?.length)
    ) {
      recordsCreated += await saveQuickBooksDesktopDetailOpenSnapshots(
        companyId,
        frequency,
        payload,
      );
      ...
      recordsCreated += await saveAROpenInvoices(
        ...
        snapshotDate,
        frequency,
        Array.isArray(payload.arOpenInvoices) ? payload.arOpenInvoices : [],
      );
    }
  } catch (error) {
    errors.push(
      `arOpenInvoices: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    if (
      ...
      recordsCreated += await saveAPOpenBills(
        ...
        snapshotDate,
        frequency,
        Array.isArray(payload.apOpenBills) ? payload.apOpenBills : [],
      );
    }
  } catch (error) {
    errors.push(
      `apOpenBills: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    recordsCreated += await saveARPayments(
      companyId,
      Array.isArray(payload.arPayments) ? payload.arPayments : [],
    );
  } catch (error) {
    errors.push(
      `arPayments: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
  try {
    recordsCreated += await saveAPPayments(
      companyId,
      Array.isArray(payload.apPayments) ? payload.apPayments : [],
    );
  } catch (error) {
    errors.push(
      `apPayments: ${error instanceof Error ? error.message : "failed to persist"}`,
    );
  }
