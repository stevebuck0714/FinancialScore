...
function qbdTxnNumber(
  record: Record<string, unknown>,
  fallbackPrefix: string,
  index: number,
): string {
  return (
    firstString(record.RefNumber, record.TxnNumber, record.TxnID) ||
...
function invoiceOpenBalance(record: Record<string, unknown>): number {
  ...
  if (hasField(record, "BalanceRemaining"))
    return toNumber(record.BalanceRemaining);
  if (isExplicitlyUnpaid(record) && hasField(record, "AmountDue"))
    return toNumber(record.AmountDue);
  return 0;
}
...
function buildSourceTransaction(
  record: Record<string, unknown>,
  fallback: string,
): string {
  return (
    firstString(record.TxnID, record.EditSequence, record.RefNumber) || fallback
  );
}

function reportColNumber(
  record: Record<string, unknown>,
  index: number,
): number {
  const colData = Array.isArray(record.colData)
    ? record.colData.map(asRecord)
    : [];
...
function reconcileAgingBucketsToTotal(
  ...
  let bucketTotal = Object.values(buckets).reduce(
    (sum, value) => sum + value,
    0,
  );
  let excess = bucketTotal - explicitTotal;
  if (excess > 0) {
    const bucketNamesBySize = Object.keys(buckets).sort(
...
function buildAgingSummaryFromReport(
  ...
  const totalRows = records.filter(
    (record) => String(record.rowKind || "") === "TotalRow",
  );
...
function normalizeDateRange(value: unknown): QbdPayloadSession["dateRange"] {
  ...
  return {
    ...
    requestedAt:
      typeof src.requestedAt === "string" ? src.requestedAt : undefined,
  };
}
...
export async function loadQuickBooksDesktopBackfillResponse(
  ...
  const rows = await prisma.$queryRaw<
    Array<{ payload: unknown; createdAt: Date }>
  >`
...
export async function loadQuickBooksDesktopBackfillSession(
  ...
  for (const job of jobs) {
    responses[job.requestName] = await loadQuickBooksDesktopBackfillResponse(
      job.id,
    );
  }
...
export async function loadQuickBooksDesktopBackfillPayloads(
  ...
  operationalPayload: QbDesktopOperationalPayload;
} | null> {
  const session = await loadQuickBooksDesktopBackfillSession(
    companyId,
    metadata,
  );
...
export function buildQuickBooksDesktopFinancialPayload(
  session: QbdPayloadSession,
  ...
  for (const [requestName, response] of Object.entries(
    session.responses || {},
  )) {
    payload[requestName] = {
      [RET_TAG_BY_REQUEST[requestName] || requestName.replace(/Query$/, "Ret")]:
        response.records || [],
    };
  }
...
export function buildQuickBooksDesktopOperationalPayload(
  session: QbdPayloadSession,
): QbDesktopOperationalPayload {
  ...
  const billPaymentChecks =
    session.responses.BillPaymentCheckQuery?.records || [];
  const billPaymentCreditCards =
    session.responses.BillPaymentCreditCardQuery?.records || [];
  const items = session.responses.ItemQuery?.records || [];
  const arAgingReport =
    session.responses.ARAgingSummaryReportQuery?.records || [];
  const apAgingReport =
    session.responses.APAgingSummaryReportQuery?.records || [];
  const asOfDate =
    qbdDate(session.dateRange.endDate) || new Date().toISOString().slice(0, 10);

  const cash = accounts
    .filter((account) => {
      const type = String(
        account.AccountType || account.SpecialAccountType || "",
      ).toLowerCase();
  ...
  const customerSalesById = new Map<
    string,
    {
      customerId: string;
      customerName: string;
  ...
  for (const [index, invoice] of invoices.entries()) {
    ...
    const revenue = toNumber(
      invoice.Subtotal || invoice.TotalAmount || invoice.Amount,
    );
    ...
    if (arBalance > 0) {
      arOpenInvoices.push({
        ...
        status:
          firstString(invoice.IsPaid).toLowerCase() === "true"
            ? "PAID"
            : "OPEN",
        currencyCode:
          firstString(asRecord(invoice.CurrencyRef).FullName) || "USD",
    ...
    const lines = Array.isArray(invoice.InvoiceLineRet)
    ...
    for (const line of lines) {
      ...
      const itemName =
        item.name || String(lineRecord.Desc || "") || "Unknown Item";
  ...
  const apOpenBills = bills
    .map((bill, index) => {
      ...
      return {
        ...
        status:
          firstString(bill.IsPaid).toLowerCase() === "true" ? "PAID" : "OPEN",
        currencyCode: firstString(asRecord(bill.CurrencyRef).FullName) || "USD",
        amountCurrency: toNumber(
          bill.AmountDue || bill.Amount || bill.TotalAmount || amountDue,
        ),
        amountHome: toNumber(
          bill.Amount || bill.TotalAmount || bill.AmountDue || amountDue,
        ),
  ...
  const arPayments = receivePayments.flatMap((payment, paymentIndex) => {
    ...
    const currencyCode =
      firstString(asRecord(payment.CurrencyRef).FullName) || "USD";
    const sourceTransaction = buildSourceTransaction(
      payment,
      `receive-payment:${paymentIndex}`,
    );
    const applied = asArray(payment.AppliedToTxnRet).filter((row) =>
      String(row.TxnType || "")
        .toLowerCase()
        .includes("invoice"),
    );
    ...
    return applied.map((appliedTxn, appliedIndex) => ({
      ...
      currencyCode,
      paidAmountCurrency:
        toNumber(appliedTxn.Amount) || toNumber(payment.TotalAmount),
      paidAmountHome:
        toNumber(appliedTxn.Amount) || toNumber(payment.TotalAmount),
  ...
  const apPayments = [...billPaymentChecks, ...billPaymentCreditCards].flatMap(
    (payment, paymentIndex) => {
      const vendor = getRef(payment, "PayeeEntityRef");
      const vendorFallback =
        vendor.id || vendor.name ? vendor : getRef(payment, "VendorRef");
      const paymentDate = qbdDate(payment.TxnDate);
      const currencyCode =
        firstString(asRecord(payment.CurrencyRef).FullName) || "USD";
      const sourceTransaction = buildSourceTransaction(
        payment,
        `bill-payment:${paymentIndex}`,
      );
      const applied = asArray(payment.AppliedToTxnRet).filter((row) =>
        String(row.TxnType || "")
          .toLowerCase()
      ...
      return applied.map((appliedTxn, appliedIndex) => ({
        ...
        currencyCode,
        paidAmountCurrency:
          toNumber(appliedTxn.Amount) ||
          toNumber(payment.Amount || payment.TotalAmount),
        paidAmountHome:
          toNumber(appliedTxn.Amount) ||
          toNumber(payment.Amount || payment.TotalAmount),
        sourceTransaction: `${sourceTransaction}:${appliedIndex}`,
  ...
  return {
    ...
    cash,
    // An invoice query covers only the requested transaction range, not the
    // complete open-invoice ledger. Never synthesize an AR aging total from it.
