...
export async function GET(request: NextRequest) {
  try {
    ...
    if (cacheContext) {
      const cachedPayload = await readDerivedApiCache<{ records: any[] }>(
        cacheContext,
      );
      if (cachedPayload) {
        if (cachedPayload.records?.length > 0) {
          await auditFinancialAccess(
            ...
            cachedPayload.records[0].id,
            companyId,
          );
        }
        const presented = await presentCompanyJson(
          request,
          companyId,
          cachedPayload,
        );
        return NextResponse.json(presented, {
          headers: privateCacheHeaders(FINANCIALS_CACHE_TTL_SECONDS, 300),
        });
      }
    }
...
export async function POST(request: NextRequest) {
  try {
        ...
        monthlyData: {
          create: monthlyData.map((month: any) => {
            ...
            if (isNaN(parsedDate.getTime())) {
              console.error(
                `Invalid date for month record: ${dateValue}`,
                month,
              );
            ...
            const derivedRevenueBreakdown =
              ...
              typeof month.revenueBreakdown === "object"
                ? month.revenueBreakdown
            ...
            const derivedExpenseBreakdown =
              ...
              typeof month.expenseBreakdown === "object"
                ? { ...month.expenseBreakdown }
                : {};
            if (Number(month.nonOperatingExpense || 0) !== 0) {
              derivedExpenseBreakdown.nonOperatingExpense = Number(
                month.nonOperatingExpense || 0,
              );
            }
            ...
            return {
              ...
              revenueBreakdown:
                Object.keys(derivedRevenueBreakdown).length > 0
                  ? derivedRevenueBreakdown
                  : null,
              expense: month.expense || 0,
              expenseBreakdown:
                Object.keys(derivedExpenseBreakdown).length > 0
                  ? derivedExpenseBreakdown
                  : null,
              ...
              cogsBreakdown:
                Object.keys(derivedCogsBreakdown).length > 0
                  ? derivedCogsBreakdown
                  : null,
    ...
    console.log(
      `✅ Financial record created with ${financialRecord.monthlyData.length} months of data`,
    );
