WITH payload AS (
  SELECT
    ...
    jsonb_build_array(
      ...
      jsonb_build_object('module','Sales','miProgram','SLCohdrs','endpointPath','/APR_PRD/CSI/IDORequestService/ido/load/SLCohdrs?properties=CoNum,CustNum,DerCustNoName,Stat,OrderDate,DueDate&recordCap=1000','transactions',jsonb_build_array('CSI_LOAD'),'mongooseConfig','TMSManager','site','','enabled',true),
      ...
      jsonb_build_object('module','Vendors','miProgram','SLVendors','endpointPath','/APR_PRD/CSI/IDORequestService/ido/load/SLVendors?properties=VendNum,Name,RecordDate,LastPaid,LastPurch,PayYtd,PayLstYr,PurchYtd,PurchLstYr,CurrCode,TermsCode,PayType,Phone,Stat,VadAddr_1,VadCity,VadState,VadZip,VadCountry,DerAgeBal1,DerAgeBal2,DerAgeBal3,DerAgeBal4,DerAgeBal5,DerAgeBal6,DerNewAmount,DerOldAmount&recordCap=1000','transactions',jsonb_build_array('CSI_LOAD'),'mongooseConfig','TMSManager','site','','enabled',true),
