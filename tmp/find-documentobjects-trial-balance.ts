...
function containsTrial(value: unknown): boolean {
  const t = String(value || '').toLowerCase();
  return t.includes('trialbalance') || t.includes('trial balance');
}
...
async function main() {
  ...
  const path = '/APR_PRD/CSI/IDORequestService/ido/load/DocumentObjects?properties=*&recordCap=1000';
  ...
  const hits = items.filter((item) => Object.values(item).some((v) => containsTrial(v)));
  console.log(
    JSON.stringify(
      {
