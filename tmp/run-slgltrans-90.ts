...
const lookbackDays = Number(process.argv[4] || 90);
...
async function resolveSlgltransOffset(): Promise<number> {
  ...
  for (let i = 0; i < configured.length; i += 1) {
    ...
    if (!enabled) continue;
    if (normalizeProgramId(normalized) === 'SLGLTRANS') return i;
  }
  throw new Error('SLGLTRANS program not found in enabled CSI metadata.');
}

async function main() {
  const offset = await resolveSlgltransOffset();
  const endDate = new Date();
  endDate.setUTCHours(23, 59, 59, 999);
  const days = Number.isFinite(lookbackDays) && lookbackDays > 0 ? Math.floor(lookbackDays) : 90;
  ...
  while (true) {
    loop += 1;
    if (loop > 50) throw new Error('Aborting SLGLTRANS test after 50 loops');
  ...
  console.log(JSON.stringify({ done: true, lookbackDays: days, offset, totalRecords }, null, 2));
}
